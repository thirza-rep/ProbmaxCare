...
class ChatController extends Controller
{
    ...
    public function chat(Request $request)
    {
        ...
        if (!$apiKey) {
            return $this->errorResponse('Maaf, layanan AI belum dikonfigurasi. Silakan hubungi administrator.', 500);
        }
        ...
        if ($lastCheck) {
            $userContext .= "Status Kesehatan Mental Terakhir (Cek Harian): {$lastCheck->stress_level} (Skor:
{$lastCheck->total_score})\n";
        } else {
...
// or as a special context for the current message.
        ...
        try {
            ...
            if ($response->successful()) {
                $data = $response->json();
                $aiResponse = $data['candidates'][0]['content']['parts'][0]['text'] ?? 'Maaf, saya tidak bisa memproses pesan Anda saat
ini.';
    ...
    private function getFallbackResponse($message)
    {
        ...
        if (str_contains($message, 'stres') || str_contains($message, 'tertekan')) {
            return "Saya mengerti perasaan stres bisa sangat berat. Cobalah teknik pernapasan dalam atau istirahat sejenak. Kamu
juga bisa gunakan fitur 'Cek Harian' untuk memahami kondisimu lebih baik, atau 'Buat Janji' untuk konsultasi dengan
profesional.";
        }

        if (str_contains($message, 'cemas') || str_contains($message, 'khawatir')) {
            return "Perasaan cemas itu wajar, tapi jangan biarkan menguasai dirimu. Coba fokus pada hal-hal yang bisa kamu kontrol.
Fitur 'PMC Game' bisa membantu kamu tracking mood harian. Jika cemas berlanjut, pertimbangkan untuk konsultasi via 'Buat
Janji'.";
        }

        if (str_contains($message, 'sedih') || str_contains($message, 'depresi')) {
            return "Saya turut prihatin mendengarnya. Perasaan sedih yang berkepanjangan perlu perhatian serius. Yuk gunakan fitur
'Cek Harian' untuk monitor kondisimu, dan jangan ragu untuk 'Buat Janji' konseling dengan profesional kami.";
        }
        ...
        return "Terima kasih sudah berbagi. Saya di sini untuk mendengarkan. Kalau kamu butuh bantuan lebih lanjut, coba gunakan
fitur 'Cek Harian' atau 'Buat Janji' untuk konsultasi dengan profesional.";
    }
}